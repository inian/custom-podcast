...
def initFeed(podcastFile):
    ...
    for entry in d.entries:
        print entry['links']
        fi = FeedItem(id=entry['id'], title=entry['title'], link=entry['links'][-1]['href'], mimeType=entry['links'][-1]['type'], length=entry['links'][-1]['length'], description=entry['summary'])
        f.addEntry(fi)
    return f
...
def uploadToS3(filename, filepath):
    s3 = boto3.client('s3')
    s3.upload_file(filepath, "custom-podcasts", filename, ExtraArgs={'ACL': 'public-read'})
...
def lambda_handler(event, context):
    ...
    newFeedItem = FeedItem(id=unique_name, title = videoName, link = "https://s3.amazonaws.com/custom-podcasts/" + filename, mimeType = 'video/mp4',length = str(os.path.getsize(filepath)), description = 'Well what do you want me to say')
